...
def get_openai_client():
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "or pass a key to the OpenAI client before calling the API."
        )

    base_url = os.getenv("OPENAI_BASE_URL")
    if base_url:
        # ✅ Correct parameter name is base_url, not api_base
...
"""
    ...
    try:
        ...
        # Missing API key: show a clear message and fall back
        print("⚠ OpenAI API key missing:", re)
        text = f"""
<html>
  <head><title>Fallback App</title></head>
  <body>
    <h1>Hello (fallback)</h1>
    <p>This app was generated as a fallback because the OpenAI API key is not configured. Brief: {brief}</p>
  </body>
</html>
